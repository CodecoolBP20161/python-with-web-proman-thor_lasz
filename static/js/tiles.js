...
function createTile() {
  ...
  var colorList = ["#daa520", "#cd0000", "#4682b4", "#2e8b57"];

// Random tileing with predefined colours
// http://www.atrandom.iansharpe.com/random-tiling.php
// rgbToHex
if(tiles.length > 1){
    do{
        var tileColor = colorList[Math.floor(Math.random() * colorList.length)];
        tiles[tiles.length-1].style.backgroundColor = tileColor;
        var current = tiles[tiles.length-1].style.backgroundColor;
        var previous = tiles[tiles.length-2].style.backgroundColor;
  ...
  // var tileColor = colorList[Math.floor(Math.random() * colorList.length)];
  // tiles[tiles.length-1].style.backgroundColor = tileColor;
  // console.log(tiles[tiles.length-1].style.backgroundColor);
